...
> 版本: 1.4
> 日期: 2026-05-18
> 状态: 评审完成（已纳入两轮对抗式评审意见）
> 关联: 用户需求 — Agent 生成网页的发布与浏览
> 评审: 对抗式专家评审 × 2（见 `docs/reviews/spec-html-render-review.md` / `spec-html-render-review-v2.md`）
...
检测范围为静态 HTML 的属性引用（`href` / `src` / `action`），不覆盖 JS 动态创建的资源请求（如 `fetch('./data.json')`）。此为已知限制，可接受。
...
| iframe 内 fetch 外泄 | ⚠️ 已知，接受（结构性限制）| `allow-scripts` 允许 fetch；Blob URL 不经过后端，后端 CSP 无法管控 iframe 内请求，无后续收紧路径 |
...
### P3 — 验证
- [x] 截图：HTML 渲染效果（桌面端 + 移动端 + 暗色模式）✅
- [x] 相对路径警告条正确显示 ✅
- [x] 多文件切换：.html→iframe，.css→CodeViewer，Wrap 按钮随切换显示/隐藏 ✅
- [x] iframe 内 `<title>` 不污染外层 `document.title`（验证：outer title = 'PeekView'）✅
- [~] ECharts CDN / Tailwind CDN 外部资源加载：沙盒环境网络白名单限制，CDN 被拦截，**需在真实部署环境验证**
- [~] iOS Safari + Android Chrome 滚动行为：无真实移动设备，**需在真实设备验证**
- [~] Blob URL iframe 不受父文档 CSP 限制：需后端配置 CSP 才能验证，**需在真实部署环境验证**
